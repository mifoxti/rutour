...
public class PlaceAdapterModern extends RecyclerView.Adapter<PlaceAdapterModern.ViewHolder> {

    private List<Place> places;
    private List<Place> filteredPlaces; // Добавлено для поддержки фильтрации
    private DBHelper dbHelper;
    private Context context;
    private boolean isCreateTab; // Добавлено для определения контекста

    public PlaceAdapterModern(Context context, boolean isCreateTab) {
        this.context = context;
        this.dbHelper = new DBHelper(context);
        this.places = new ArrayList<>();
        this.filteredPlaces = new ArrayList<>(); // Инициализируем список для фильтрации
        this.isCreateTab = isCreateTab; // Инициализируем флаг контекста
    ...
    @Override
    public PlaceAdapterModern.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.gallery_item, parent, false);
        return new ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(PlaceAdapterModern.ViewHolder holder, int position) {
        ...
        // Обработчик нажатия на карточку, показываем диалог только если это вкладка "Создать"
        if (isCreateTab) {
            holder.itemView.setOnClickListener(v -> {
                AlertDialog.Builder builder = new AlertDialog.Builder(context);
                builder.setTitle(place.getName())
                        .setItems(new CharSequence[]{"Изменить", "Удалить", "Отмена"}, (dialog, which) -> {
                            switch (which) {
                                case 0: // Изменить
                                ...
                                case 1: // Удалить
                                    ...
                                    loadPlacesFromDatabase();
                                    Snackbar.make(holder.itemView, "Место удалено", Snackbar.LENGTH_SHORT).show();
                                    break;
                                case 2: // Отмена
                                    dialog.dismiss();
                                    break;
